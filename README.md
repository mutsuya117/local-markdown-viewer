...
- **オフライン対応**:
  ...
  - ✅ **Mermaidダイアグラム**: 完全オフライン（JavaScriptライブラリを埋め込み、SVGも保存）
...
- **含まれる機能**:
  ...
  - ✅ ダークモード切り替え（初期値：ライト）
  - ✅ 印刷機能
  - ✅ Mermaidダイアグラム（JavaScriptライブラリ埋め込み、完全オフライン動作）
  - ✅ シンタックスハイライト（既に適用済み）
...
- **オフライン表示**:
  ...
  - ✅ **Mermaidダイアグラム**: 完全オフライン（JavaScriptライブラリとSVGを埋め込み）
  - ⚠️ **KaTeX数式**: ほぼオフライン（CSSを埋め込み、フォントのみCDN推奨）
- **推奨環境**: インターネット接続がある環境（KaTeXフォントで最高品質）

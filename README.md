...
- **軽量**: 必要最小限のCDNライブラリのみ読み込み（Mermaid JS、KaTeX CSS）
- **画像の自動埋め込み**:
...
- エクスポートされたHTMLを開くには**インターネット接続が必要です**：
  - Mermaidライブラリ（約800KB）が常にCDNから読み込まれます
  - KaTeX機能がONの場合、KaTeX CSS（約80KB）もCDNから読み込まれます
...
**注意**: エクスポート機能では、MermaidライブラリとKaTeX CSSはCDN（cdn.jsdelivr.net）から読み込まれます。

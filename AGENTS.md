...
2. **シンタックスハイライト**
   - 140+のプログラミング言語に対応
   - GitHubスタイルのカラースキーム
   - **罫線テーブルの桁ずれ防止**: コードブロックでOS標準のCJK等幅フォントを優先
     - フォントの同梱・ダウンロードは行わない（オフライン・エクスポートでもそのまま機能）
     - CJK等幅フォント（Win: BIZ UDGothic/MS Gothic/NSimSun、mac: Osaka-Mono、
       Linux: Noto Sans Mono CJK）は「半角=1:全角=2」の正確な比率を持つため、先頭に置くと
       コードブロック全体が単一書体で描画され、日本語・中国語・英語混在でも罫線が揃う
     - UI言語で先頭フォントを切替（簡体字UI=中国語フォント優先、それ以外=日本語フォント優先）
     - 合字（ligature）は無効化し、罫線が連結されないようにしている
     - ベストエフォート方式: CJK等幅フォントが1つも無い環境では完全な整列は保証されない

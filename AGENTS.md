...
2. **シンタックスハイライト**
   - 140+のプログラミング言語に対応
   - GitHubスタイルのカラースキーム
   - **罫線テーブルの桁ずれ防止**: コードブロック（`pre code`）でOS標準のCJK等幅フォントを優先
     - 適用範囲はコードブロックのみ。インラインコード（`` `code` ``）は通常の等幅フォントのまま
       （罫線揃えが必要なのはコードブロックで、インラインまでCJK書体にすると英数字の見栄えが劣化するため）
     - フォントの同梱・ダウンロードは行わない（オフライン・エクスポートでもそのまま機能）
     - CJK等幅フォント（Win: BIZ UDGothic/MS Gothic/NSimSun/MingLiU、mac: Osaka-Mono、
       Linux: Noto Sans Mono CJK）は「半角=1:全角=2」の正確な比率を持つため、先頭に置くと
       コードブロック全体が単一書体で描画され、日本語・中国語・英語混在でも罫線が揃う
     - UI言語で先頭フォントを切替（日本語=日本語フォント、簡体字=簡体字フォント、繁体字=繁体字フォント優先）

...
### 同梱フォント
- **Sarasa Mono** v1.0.39（SIL OFL 1.1） - コードブロック用の等幅フォント
  - CJK:半角ラテン=2:1の正確な等幅で、罫線（─│┌など）も半角
  - 罫線テーブルが日本語・中国語・英語混在でもずれないようにするため同梱
  - J（日本語字形）/ SC（簡体字字形）の2バリアントを同梱し、UI言語で優先順を切替
...
│   └── fonts/             # 同梱フォント（KaTeX用 + Sarasa Mono J/SC）
├── styles/               # カスタムCSSスタイル
...
2. **シンタックスハイライト**
   - 140+のプログラミング言語に対応
   - GitHubスタイルのカラースキーム
   - **罫線テーブルの桁ずれ防止**: コードブロックに等幅フォント Sarasa Mono を同梱
     - 罫線（─│┌┐└┘├┤┬┴┼ ╔═╗ など）を使った表が、日本語・中国語・英語
       混在でもずれない（CJK=全角、罫線/半角ラテン=半角で正確に1:2）
     - UI言語に応じて字形バリアントを切替（簡体字UI=SC優先、それ以外=J優先）
     - 通常表示は同梱フォントを参照、HTMLエクスポートはCDN（jsDelivr）を参照
